...
## ⚙️ Рекомендации по Гиперпараметрам

### Для Logistic Regression

```python
# Параметры TF-IDF:
max_features: 5000-20000        # Больше = больше capacity
ngram_range: (1,1), (1,2), (1,3) # Выше = больше контекста
min_df: 2-10                     # Ниже = больше токенов

# Регуляризация:
C: 0.1-10.0                      # Ниже = сильнее регуляризация
```
...
```python
# Архитектура:
embedding_dim: 50-200            # Больше = рашше представление
num_filters: 50-200              # Больше = больше паттернов
kernel_sizes: (2,3,4), (2,3,4,5) # Разные n-граммы

# Обучение:
batch_size: 16-64                # Баланс память/градиент
learning_rate: 0.0001-0.001      # Ниже = медленнее, стабильнее
dropout_rate: 0.3-0.7            # Больше = сильнее регуляризация
max_length: 100-300              # Больше = больше контекста
```
...
- [Kaggle Competition](https://www.kaggle.com/competitions/toxic-comment-classification-challenge)
